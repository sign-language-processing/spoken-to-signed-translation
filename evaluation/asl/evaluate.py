    ...
    for case, data in zip(cases, documents):
        if args.baseline:
            ...
            if args.glosser_url:
                ...
                result = senses_to_gloss(data, semantics=semantics.matches if semantics else None)
            output = " | ".join(" ".join(t["word"] for t in s if t["pos"] != "PUNCT") for s in result["sentences"])
        results.append(
            {
                "id": case["id"],
                "text": case["text"],
