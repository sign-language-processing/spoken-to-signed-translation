...
def _gloss_to_pose(
    sentences: list[Gloss], lexicon: str, spoken_language: str, signed_language: str, no_fingerspelling: bool = False
) -> Pose:
    backup = None if no_fingerspelling else FingerspellingPoseLookup()
...
def text_to_gloss_to_pose():
    ...
    args_parser.add_argument("--pose", type=str, required=True)
    args_parser.add_argument("--no-fingerspelling", action="store_true", help="Disable fingerspelling fallback")
    args = args_parser.parse_args()

    sentences = _text_to_gloss(args.text, args.spoken_language, args.glosser)
    pose = _gloss_to_pose(sentences, args.lexicon, args.spoken_language, args.signed_language, args.no_fingerspelling)
