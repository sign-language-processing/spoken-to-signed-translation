...
def _omit_asl(item, metadata, following=None, question=False):
...
def _asl_question_length(sentence, metadata):
    ...
    if not end or any(features.get("pos") in {"CCONJ", "SCONJ"} for features in metadata[1:]):
        return 0
    if metadata[end + 1].get("pos") not in {"PRON", "NOUN", "PROPN", "DET"}:
        return 0  # Require an explicit subject after the auxiliary; preserve uncertain WH subjects.
...
def _asl_sentence_to_gloss(sentence, metadata):
    question_length = _asl_question_length(sentence, metadata)
...
def tokens_to_gloss(
    ...
    for end, item in enumerate(tokens, 1):
        if item.word in {".", "!", "?"}:
            sentences.append(_asl_sentence_to_gloss(tokens[start:end], metadata[start:end]))
            start = end
    if start < len(tokens):
        sentences.append(_asl_sentence_to_gloss(tokens[start:], metadata[start:]))
    return sentences


def text_to_gloss(text: str, language: str, punctuation=False, signed_language=None, **unused_kwargs) -> list[Gloss]:
    if language == "en":
        if signed_language not in {None, "ase"}:
            raise ValueError("English rules support only en → ase")
        model = load_spacy_model(("en_core_web_lg", "en_core_web_md", "en_core_web_sm"))
        doc = model(text)
        tokens = [GlossItem(token.text, token.lemma_) for token in doc]
        metadata = [{"pos": token.pos_, "morphology": [token.morph.to_dict()]} for token in doc]
        sentences = tokens_to_gloss(tokens, language="en", signed_language="ase", metadata=metadata)
        if not punctuation:
            punctuation_ids = {id(item) for item, token in zip(tokens, doc) if token.is_punct}
            sentences = [[item for item in sentence if id(item) not in punctuation_ids] for sentence in sentences]
        return sentences
